/*
   Formas de criar um número
*/

const peso1 = 1.0
const peso2 = Number('2.3')

console.log(peso1, peso2)
console.log(Number.isInteger(peso1))
console.log(Number.isInteger(peso2))

/*
   Exemplos de manipulação de números
*/

const avaliacao1 = 9.871
const avaliacao2 = 6.871

const total = avaliacao1 * peso1 + avaliacao2 * peso2
const media = total / (peso1 + peso2)

console.log(media.toFixed(2)) // .toFixed() => define o n⁰ de casas decimais
console.log(media.toString()) // .toString() => transforma o valor em string
console.log(media.toString(2)) // .toString(2) => converte em binário

console.log(typeof Number) // Number com N maiúsculo é uma função!
